    ...
    @if($announcements->count() < 1) <h3 class="text-center mb-5 primary-color-text h1" style="z-index: 1;">{{__('ui.noAnnounceCardSection')}}</h3>
      ...
      <div class="row g-2">
        ...
        <div class="col-12 col-lg-6 col-xl-4 mb-4 d-flex justify-content-center ">
          <x-card :user="$announcement->user->name" :price="$announcement->price" :description="$announcement->description" :category="$announcement->category->name" :title="$announcement->title" :root="route('announce.View',$announcement->id)" :images="$announcement->images" />
        </div>
        @endforeach
      </div>
  </section>
